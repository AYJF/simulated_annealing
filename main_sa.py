...
# Simulated annealing
def simulated_annealing(matrix:list, home:int, initial_state:State, mutation_rate:float=0.01, schedule=const_schedule()):
...
def main():
    ...
    df = pd.read_excel('probarSimAnn.xlsx',sheet_name=1,header=0, skipfooter = 5, usecols='B:P')
    cities = df.columns.values.tolist()
    city_indexes = np.arange(len(cities))
    matrix = df.values
